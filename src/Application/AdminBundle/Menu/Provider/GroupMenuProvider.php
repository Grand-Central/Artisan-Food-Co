...
namespace Application\AdminBundle\Menu\Provider;
        ...
        if (empty($group['on_top'])) {
            foreach ($group['items'] as $item) {
                if (isset($item['admin']) && !empty($item['admin'])) {
                    ...
                    // skip menu item if no `list` or `edit` url is available or user doesn't have the LIST or EDIT access rights
                    if ((!$admin->hasRoute('list') || !$admin->isGranted('LIST')) && (!$admin->hasRoute('edit') || !$admin->isGranted('EDIT'))) {
                        continue;
                    }
                    ...
                    if(!$admin->hasRoute('list') || !$admin->isGranted('LIST') && ($admin->hasRoute('edit') && $admin->isGranted('EDIT'))){
                        $options = $admin->generateMenuUrl('edit', array('id' => 1), $item['route_absolute']);
                    } else {
